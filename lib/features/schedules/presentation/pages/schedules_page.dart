...
class _SchedulesPageState extends State<SchedulesPage> {
  ...
  List<MockSchedule> get _filtered {
    var items = MockSchedule.samples.toList();

    if (_activeFilter != 'Todos') {
      items = items.where((s) {
        return switch (_activeFilter) {
          'Aguardando aprovação' =>
            s.status == ScheduleStatus.awaitingClientApproval,
          'Aguardando confirmação' =>
            s.status == ScheduleStatus.awaitingDriverConfirmation,
          'Agendado' => s.status == ScheduleStatus.scheduled,
          _ => true,
        };
      }).toList();
    }
    ...
    items.sort((a, b) {
      final aPriority =
          a.status == ScheduleStatus.awaitingDriverConfirmation ? 0 : 1;
      final bPriority =
          b.status == ScheduleStatus.awaitingDriverConfirmation ? 0 : 1;
      final cmp = aPriority.compareTo(bPriority);
      return cmp != 0 ? cmp : a.scheduledDate.compareTo(b.scheduledDate);
    });
                  ...
                  child: items.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (_, i) => GestureDetector(
                            ...
                            child: _ScheduleCard(schedule: items[i]),
                          ),
                        ),
                ),
              ],
